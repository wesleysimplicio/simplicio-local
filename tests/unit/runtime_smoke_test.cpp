...
int main() {
  ...
  ok &= Expect(expected_mode == probe.recommendedMode,
               "recommended mode should match memory tier");
  ...
  ok &= Expect(adapters.size() >= 7,
               "adapter registry should expose sprint 02-08 families");

  const us4::BackendSelection backend =
      us4::SelectBackend(probe, context.mode(), *adapters.front());
  ok &= Expect(!backend.reason.empty(),
               "backend selection should produce a reason");

  us4::UnifiedAllocator allocator;
  const auto allocation = allocator.Allocate(128, false);
  ok &= Expect(allocation->bytes.size() == 128,
               "unified allocator should reserve bytes");
  const auto sharedAllocation = allocator.Allocate(256, true);
  ok &= Expect(sharedAllocation->visibility ==
                   us4::AllocationVisibility::kUnifiedShared,
  ...
  if (probe.hasMlx) {
    ...
    ok &= Expect(context.mlxBridge().EvaluateLastPlan(),
                 "mlx bridge should evaluate built plan");
  }
  ...
  ok &= Expect(prefixCache.EntryCount() == 1,
               "prefix cache should retain entries");

  us4::SsdColdStore coldStore;
  ok &= Expect(coldStore.Flush("prompt", {1.0F, 2.0F}),
               "cold store should flush");
  ok &= Expect(coldStore.Restore("prompt").has_value(),
               "cold store should restore");

  us4::Summarizer summarizer;
  ok &= Expect(!summarizer.Summarize({1.0F, 3.0F, 5.0F}).empty(),
               "summarizer should produce a summary");
  ...
  ok &= Expect(ropeTensor.DataAsFloat32()[0] != 1.0F,
               "rope should rotate tensor values");
  ...
  query.MutableDataAsFloat32()[2] = 0.0F;
  query.MutableDataAsFloat32()[3] = 1.0F;
  key.MutableDataAsFloat32()[0] = 1.0F;
  key.MutableDataAsFloat32()[1] = 0.0F;
  key.MutableDataAsFloat32()[2] = 0.0F;
  key.MutableDataAsFloat32()[3] = 1.0F;
  value.MutableDataAsFloat32()[0] = 2.0F;
  value.MutableDataAsFloat32()[1] = 0.0F;
  value.MutableDataAsFloat32()[2] = 0.0F;
  value.MutableDataAsFloat32()[3] = 4.0F;
  std::string attentionError;
  ok &= Expect(us4::GqaAttention(query, key, value, 2, 1, out, &attentionError),
               "gqa attention should succeed");
  ok &= Expect(out.DataAsFloat32()[0] > out.DataAsFloat32()[2],
               "gqa attention should keep head-wise outputs distinct");

  us4::Router router;
  ok &= Expect(router.TopK({0.1F, 0.9F, 0.4F}, 2).size() == 2,
               "router should return top-k experts");

  us4::ExpertPager expertPager(1);
  expertPager.Touch("expert-a");
  expertPager.Touch("expert-b");
  ok &= Expect(expertPager.ResidentCount() == 1,
